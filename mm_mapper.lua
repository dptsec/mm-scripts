...
* Added code to resize the window by dragging its bottom right corner
  (adapted from Fiendish's/Lasher's code for Aardwolf)

2014-07-22
* added code to show a background image (in case someone wants to use this feature, the image file needs to be RGB, 8 bits per channel, named mm_mapper_bg.png, and placed in MUSHclient/worlds)
  (adapted from Fiendish's code for Aardwolf)
...
2013-01-26
* Added support for tprt (teleport) and prtl (portal) exits
...
local supplied_get_exits_for_path_batch
local supplied_get_reverse_exits_batch
local supplied_get_path_cache_generation
local room_click
local room_mouseover
local room_cancelmouseover
local speedwalk_mismatch
...
local path_cache_generation

local function synchronize_path_cache ()
  if not supplied_get_path_cache_generation then
    return
  end

  local generation = supplied_get_path_cache_generation ()
  if generation ~= nil and generation ~= path_cache_generation then
    reset_path_caches ()
    path_cache_generation = generation
  end
end
...
local function normalize_room (uid, room)
...
local function get_room (uid)
  return normalize_room (uid, supplied_get_room (uid))
end -- get_room
...
local function cached_path_room (uid)
  ...
  return nil
end -- cached_path_room

local function ensure_path_rooms (uids)
  local missing = {}
  local seen = {}
  ...
  if supplied_get_rooms_for_path_batch and supplied_get_exits_for_path_batch then
    local batch = supplied_get_rooms_for_path_batch (missing) or {}
  ...
  for _, uid in ipairs (missing) do
    cached_path_room (uid)
  end -- for
end -- ensure_path_rooms
...
local function materialize_draw_path (node)
  local reversed_path = {}
  while node and node.dir do
    table.insert (reversed_path, { dir = node.dir, uid = node.uid })
    node = node.parent
  end

  local path = {}
  for i = #reversed_path, 1, -1 do
    table.insert (path, reversed_path [i])
  end
  return path
end
...
local function draw_room (uid, path, x, y)
      ...
      elseif exit_uid == uid then
        ...
        if (not show_other_areas and rooms [exit_uid].area ~= current_area) or
          ...
          if plan_to_draw [exit_uid] and plan_to_draw [exit_uid] ~= next_coords then
            ...
            local new_path = { parent = path, dir = dir, uid = exit_uid }
  ...
  speedwalks [uid] = materialize_draw_path (path)  -- so we know how to get here
...
local function sort_directions (exits)
  ...
  table.sort (dirs, function (a, b)
    local arank = direction_rank (a)
    local brank = direction_rank (b)
    if arank == brank then
      return a < b
    end -- if same rank
    return arank < brank
...
local function room_can_be_path_step (uid)
  return is_safewalk_check_ok(uid)
    and can_be_used_on_speedwalks(uid)
end -- room_can_be_path_step

local function build_forward_path (start_uid, meeting_uid, previous_rooms)
  local reversed_path = {}
  local current = meeting_uid

  while current ~= start_uid do
    local previous = previous_rooms[current]
    if not previous then
      break
    end -- if no previous room
    table.insert(reversed_path, { dir = previous.dir, uid = current })
  ...
  return path
end -- build_forward_path

local function build_exact_path (start_uid, meeting_uid, previous_forward, next_backward)
  local path = build_forward_path (start_uid, meeting_uid, previous_forward)
  local current = meeting_uid

  while next_backward [current] do
    local step = next_backward [current]
    table.insert(path, { dir = step.dir, uid = step.to })
    current = step.to
  end -- while

  return path
end -- build_exact_path

local function get_sorted_forward_exits_for_exact_path (uid)
  local cached = exact_path_forward_exits [uid]
  if cached then
    return cached
  end -- if cached

  local room = cached_path_room (uid)
  local exits = {}

  if room then
    for _, dir in ipairs(sort_directions(room.exits)) do
      table.insert(exits, { dir = dir, uid = room.exits [dir] })
    end -- for each exit
  end -- if room

  exact_path_forward_exits [uid] = exits
  return exits
end -- get_sorted_forward_exits_for_exact_path

local function sort_reverse_exits_for_exact_path (reverse_exits)
  table.sort(reverse_exits, function (a, b)
    local arank = direction_rank (a.dir)
    local brank = direction_rank (b.dir)
    if arank == brank then
      if a.dir == b.dir then
        return tostring(a.fromuid) < tostring(b.fromuid)
  ...
  return reverse_exits
end -- sort_reverse_exits_for_exact_path

local function sort_forward_exits_for_exact_path (forward_exits)
  table.sort(forward_exits, function (a, b)
    local arank = direction_rank (a.dir)
    local brank = direction_rank (b.dir)
    if arank == brank then
      if a.dir == b.dir then
  ...
  return forward_exits
end -- sort_forward_exits_for_exact_path

local function get_sorted_reverse_exits_for_exact_path (uid)
  local cached = exact_path_reverse_exits [uid]
  if cached then
    return cached
  end -- if cached

  local reverse_exits = supplied_get_reverse_exits (uid) or {}
  exact_path_reverse_exits [uid] = sort_reverse_exits_for_exact_path (reverse_exits)
  return exact_path_reverse_exits [uid]
end -- get_sorted_reverse_exits_for_exact_path

local function ensure_sorted_forward_exits_for_exact_path (uids)
  local missing = {}
  for _, uid in ipairs (uids) do
    if not exact_path_forward_exits [uid] then
      table.insert(missing, uid)
    end -- if missing
  end -- for
  ...
  if supplied_get_exits_for_path_batch then
    local batch = supplied_get_exits_for_path_batch (missing) or {}
    for _, uid in ipairs (missing) do
      exact_path_forward_exits [uid] = sort_forward_exits_for_exact_path (
        batch [uid] or batch [tostring (uid)] or {})
    end -- for
    return
  end -- if batch callback available

  for _, uid in ipairs (missing) do
    get_sorted_forward_exits_for_exact_path (uid)
  end -- for
end -- ensure_sorted_forward_exits_for_exact_path

local function ensure_sorted_reverse_exits_for_exact_path (uids)
  local missing = {}
  for _, uid in ipairs (uids) do
    if not exact_path_reverse_exits [uid] then
      table.insert(missing, uid)
    end -- if missing
  end -- for
  ...
  if supplied_get_reverse_exits_batch then
    local batch = supplied_get_reverse_exits_batch (missing) or {}
    for _, uid in ipairs (missing) do
      exact_path_reverse_exits [uid] = sort_reverse_exits_for_exact_path (
        batch [uid] or batch [tostring (uid)] or {})
    end -- for
    return
  end -- if batch callback available
  ...
  for _, uid in ipairs (missing) do
    get_sorted_reverse_exits_for_exact_path (uid)
  end -- for
...
local function changed_room (uid)
  ...
  if current_speedwalk then
    ...
    if (uid ~= expected_room) then
      local recovery_dir = nil
      if type (speedwalk_mismatch) == "function" then
        local ok, handled = pcall(speedwalk_mismatch, {
          fromuid = current_room,
          dir = next_speedwalk_dir or next_dir,
        ...
        end -- if mismatch handler failed
...
function find_paths (uid, f)
  ...
  while (not done) and #particles > 0 and depth < config.SCAN.depth do
    ...
    for i, current_uid in ipairs (particles) do
      ...
      if rooms [current_uid] then
        ...
        for _, dir in ipairs(sort_directions(exits)) do
          ...
          and can_exit_be_used_on_speedwalks(current_uid, dir) then
            local dest_room = cached_path_room (dest) -- make sure this room in table
            if dest_room then
              explored_rooms[dest] = true
            end -- if mapped destination

            if dest_room
            and (is_safewalk_check_ok(dest))
            and (can_be_used_on_speedwalks(dest)) then
              previous_rooms[dest] = { from = current_uid, dir = dir }

              -- if this room is in the list of destinations then save its path
              found, done = f (dest)
              if found then
                paths[dest] = { path = build_forward_path(uid, dest, previous_rooms), reason = found }
              end -- found one!

              -- make a new particle in the new room
              new_generation[#new_generation + 1] = dest
...
function find_path (uid, target_uid)
  synchronize_path_cache ()

  if uid == target_uid then
    return { path = {}, reason = true }, 0, 0
  end -- already there

  if not supplied_get_reverse_exits then
    local paths, count, depth = find_paths(uid,
       function (candidate_uid)
         return candidate_uid == target_uid, candidate_uid == target_uid
       end)
    return paths[target_uid], count, depth
  end -- no reverse lookup callback
  ...
  local forward_generation = { uid }
  local backward_generation = { target_uid }
  local forward_seen = { [uid] = true }
  local backward_seen = { [target_uid] = true }
  local previous_forward = {}
  local next_backward = {}
  local count = 0
  local depth = 0
  local meeting_uid

  while (not meeting_uid)
    and #forward_generation > 0
    and #backward_generation > 0
    and depth < config.SCAN.depth do

    depth = depth + 1

    if #forward_generation < #backward_generation then
      local new_generation = {}
      local candidate_rooms = {}
      local candidate_seen = {}
      ensure_path_rooms (forward_generation)
      ...
      for _, current_uid in ipairs (forward_generation) do
        if rooms [current_uid] then
          for _, exit in ipairs(get_sorted_forward_exits_for_exact_path (current_uid)) do
            ...
            and not candidate_seen [dest]
            and can_exit_be_used_on_speedwalks(current_uid, exit.dir) then
              table.insert(candidate_rooms, dest)
              candidate_seen [dest] = true
            end -- if candidate room
          end -- for each exit
        end -- if room
      end -- for each current room

      ensure_path_rooms (candidate_rooms)
          ...
          for _, exit in ipairs(get_sorted_forward_exits_for_exact_path (current_uid)) do
            ...
            and can_exit_be_used_on_speedwalks(current_uid, dir) then
              if rooms [dest] and room_can_be_path_step (dest) then
                ...
                if backward_seen [dest] then
                  meeting_uid = dest
                  break
                end -- if frontiers met

                table.insert(new_generation, dest)
              end -- if usable destination
            end -- if not seen
          end -- for each exit
        end -- if room
      ...
      forward_generation = new_generation

    else
      local new_generation = {}
      local candidate_rooms = {}
      local candidate_seen = {}
      ensure_path_rooms (backward_generation)
      ...
      for _, current_uid in ipairs (backward_generation) do
        if rooms [current_uid] and room_can_be_path_step (current_uid) then
          for _, reverse_exit in ipairs (get_sorted_reverse_exits_for_exact_path (current_uid)) do
            ...
            and not candidate_seen [fromuid] then
              table.insert(candidate_rooms, fromuid)
              candidate_seen [fromuid] = true
            end -- if candidate room
          end -- for each reverse exit
        end -- if room
      end -- for each current room
          ...
          for _, reverse_exit in ipairs (reverse_exits) do
            ...
            and can_exit_be_used_on_speedwalks(fromuid, dir) then
              if rooms [fromuid] and (fromuid == uid or room_can_be_path_step (fromuid)) then
                ...
                if forward_seen [fromuid] then
                  meeting_uid = fromuid
                  break
                end -- if frontiers met

                table.insert(new_generation, fromuid)
              end -- if usable predecessor
            end -- if not seen
          end -- for each reverse exit
        end -- if current room can be stepped into
      ...
      backward_generation = new_generation
    end -- which side to expand
  end -- while searching

  if not meeting_uid then
    return nil, count, depth
  end -- no path

  local path = build_exact_path (uid, meeting_uid, previous_forward, next_backward)
  if #path > config.SCAN.depth then
    return nil, count, depth
  end -- path exceeds configured depth

  return { path = path, reason = true }, count, depth
  ...
  if WindowLoadImage(win, "bg", imgpath) == 0 then
    ...
    while y < config.WINDOW.height do
       ...
       while x < config.WINDOW.width do
          WindowDrawImage (win, "bg", x, y, 0, 0, 1)  -- straight copy
          x = x + iwidth
       end
       y = y + iheight
    end
...
--  movewindow.add_drag_handler (win, 0, 0, 0, font_height + 4)
  ...
  -- insert initial room
  table.insert (rooms_to_be_drawn, add_another_room (uid, nil, config.WINDOW.width / 2, config.WINDOW.height / 2))
...
function init (t)
  ...
  end -- if path-specific room loader supplied
  supplied_get_rooms_for_path_batch = t.get_rooms_for_path_batch
  if supplied_get_rooms_for_path_batch then
    assert (type (supplied_get_rooms_for_path_batch) == "function", "'get_rooms_for_path_batch' must be a function.")
  end -- if batch path room loader supplied
  supplied_get_reverse_exits = t.get_reverse_exits
  if supplied_get_reverse_exits then
    assert (type (supplied_get_reverse_exits) == "function", "'get_reverse_exits' must be a function.")
  end -- if reverse exit loader supplied
  supplied_get_exits_for_path_batch = t.get_exits_for_path_batch
  if supplied_get_exits_for_path_batch then
    assert (type (supplied_get_exits_for_path_batch) == "function", "'get_exits_for_path_batch' must be a function.")
  end -- if batch forward exit loader supplied
  supplied_get_reverse_exits_batch = t.get_reverse_exits_batch
  if supplied_get_reverse_exits_batch then
    assert (type (supplied_get_reverse_exits_batch) == "function", "'get_reverse_exits_batch' must be a function.")
  end -- if batch reverse exit loader supplied
  supplied_get_path_cache_generation = t.get_path_cache_generation
  if supplied_get_path_cache_generation then
    assert (type (supplied_get_path_cache_generation) == "function",
      "'get_path_cache_generation' must be a function.")
  end -- if path cache generation callback supplied
  ...
  speedwalk_mismatch = t.speedwalk_mismatch -- mismatch diagnostic function
...
function invalidate_path_cache ()
  reset_path_caches ()
  if supplied_get_path_cache_generation then
    path_cache_generation = supplied_get_path_cache_generation ()
  else
    path_cache_generation = nil
  end
end
...
function add_resizer()
  ...
  while (x1 + m + 2 <= x2 - 3 and y1 + n + 1 <= y2 - 4) do
    WindowLine(win, x1 + m + 1, y2 - 4, x2 - 3, y1 + n,
               HIGHLIGHT,
               miniwin.pen_solid,
               1)
    WindowLine(win, x1 + m + 2, y2 - 4, x2 - 3, y1 + n + 1,
               SHADOW,
               miniwin.pen_solid,
               1)
    m = m + 3
  ...
  if (WindowHotspotInfo(win, "resize", 1) == nil) then
    WindowAddHotspot(win, "resizer", x, y, 0, 0,
                     "", "",
                     "mapper.resizer_mousedown", "",
                     "", "",
                     miniwin.cursor_nw_se_arrow,
                     0)
    WindowDragHandler(win,
                      "resizer",
                      "mapper.resizer_callback", "mapper.resizer_release_callback",
                      0)
  else
    WindowMoveHotspot(win, "resizer", x, y, 0,  0)
  end
end
...
function toggle_show_up_down(status)
